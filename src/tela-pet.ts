 ...
 //      R: Impede estados inválidos, como sucesso sem dados ou erro sem mensagem.
 //
 //   2. Por que `carregar()` captura a exceção em vez de deixá-la subir?
 //      R: Para transformar a falha em um estado de erro e evitar que a exceção escape.
 //
 //   3. Uma decisão de modelagem que você tomou na Prática 1 e o motivo.
 //      R: Usei tipos literais para limitar os valores possíveis e evitar dados inválidos.
 // ------------------------------------------------------------
 ...
 //     erro       → `Erro: ${mensagem}`
 // ============================================================
 export function descreverTela(
   estado: EstadoTela<Pet>,
 ): string {
   switch (estado.tipo) {
     case 'carregando':
       return 'Carregando…';
     ...
     case 'sucesso':
       return `${estado.dados.nome} · ${
         rotuloEspecie(estado.dados.especie)
       } · ${
         rotuloStatusPasseio(estado.dados.statusPasseio)
       }`;
